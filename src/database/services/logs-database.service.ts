...
export class LogsDatabaseService extends PrismaClient implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(LogsDatabaseService.name);
  ...
  async onModuleInit() {
    ...
    try {
      await this.$connect();
      this.logger.log('✅ Logs database connected successfully');
    } catch (error: any) {
      // Check if it's a database not found error (P1003)
      if (error?.code === 'P1003' || error?.message?.includes('does not exist')) {
        this.logger.warn('⚠️ Logs database does not exist yet - this might be expected during first startup');
        // Don't throw error - database might be created later
        return;
      }
      
      // For other errors, log and throw
      this.logger.error('❌ Failed to connect to logs database:', error);
      throw error;
    }
  }
